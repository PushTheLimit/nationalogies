...
<div class="taskcontainer">
	<% using (Html.BeginForm("SavePageProperties", "Pages", new { id = Model.Id }, FormMethod.Post, new { id = "pageform" })) { %>
		<p>
			<label for="Title"><%= GlobalResources.PageTitleLabel %></label>
			<%= Html.TextBox("Title", Model.Title, new { style = "width:97%;" })%>
		</p>
		<p>
			<label for="ShortDescription"><%= GlobalResources.PageUrlLabel %></label><br />
			/<%= Html.TextBox("ShortDescription", Model.ShortDescription, new { style = "width:95%;" })%>
		</p>
		<p>
		...
		<p>
			<%= Html.CheckBox("ShowInNavigation", Model.ShowInNavigation) %>
